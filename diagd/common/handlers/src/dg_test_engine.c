...
void DG_TEST_ENGINE_handler_main(DG_DEFS_DIAG_REQ_T* req)
{
    ...
    if (DG_ENGINE_UTIL_req_len_check_at_least(req, sizeof(action), rsp) == FALSE)
    {
    ...
    else
    {
        ...
        switch (action)
        {
            ...
            case DG_TEST_ENGINE_ACTION_UNSOL:
            {
                ...
                if ((DG_ENGINE_UTIL_req_remain_len_check_at_least(req, sizeof(num_unsol),
                                                                  rsp) == TRUE) &&
                    (DG_ENGINE_UTIL_rsp_data_alloc(rsp, DG_TEST_ENGINE_UNSOL_RSP_SIZE) == TRUE))
                {
            ...
            {
                /* Verify command request length and allocate response data */
                if (DG_ENGINE_UTIL_req_remain_len_check_at_least(req, 4, rsp) == TRUE)
                {
            ...
            {
                /* Verify command request length and allocate response data */
                if (DG_ENGINE_UTIL_req_remain_len_check_at_least(req, 2, rsp) == TRUE)
                {  
                    BOOL set_enable;
                    ...
                    else
                    {
                        ...
                        if (DG_AUX_ENGINE_enable_aux(aux_id, set_enable) == TRUE)
                        {
                            DG_ENGINE_UTIL_rsp_set_code(rsp, DG_RSP_CODE_CMD_RSP_GENERIC);
                        }
                        else
