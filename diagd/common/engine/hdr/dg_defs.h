...
typedef struct
{
    ...
    /* Variables for platforms using alternate timer implementation */
...
typedef struct
{
    ...
    void         (*fptr)(DG_DEFS_DIAG_REQ_T*); /**< Function pointer for opcode */
    void         (*quit)(void*);               /**< Timeout cleanup function, if user provide */
    UINT32       timeout_msec;                 /**< Timeout time in msec */
} DG_DEFS_OPCODE_ENTRY_T;

/** DIAG Status Codes.  <b>Success is 0, all other error codes must be negative</b>*/
typedef enum 
{
    DG_DEFS_STATUS_SUCCESS = 0,     /**< Success */
    DG_DEFS_STATUS_GEN_ERROR = -1,  /**< Generic error */
    DG_DEFS_STATUS_ERROR_MUX = -2,  /**< Error using the mux */
    DG_DEFS_STATUS_MALLOC_FAIL = -3 /**< Allocating memory failed */
} DG_DEFS_STATUS_T;
